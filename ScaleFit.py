...
def ScaleFit2(SourcePs,TargetPs):
    ...
    while(True):
        ...
        maxTargetIndex = 0
        maxSourceIndex = 0
        nearPointIndices = [0]*len(SourceSamplesIndices)
        for i,index in enumerate(SourceSamplesIndices):
            anser = target_kdtree.query(SourcePs[index])
            nearPointIndices[i] = anser[1]
            if anser[0] > maxD or maxD < 0:
                maxD           = anser[0]
                maxTargetIndex = anser[1]
                maxSourceIndex = index


        targetPoint = TargetPs[maxTargetIndex]
        x_scale     = targetPoint[0]*SourcePs[maxSourceIndex][0] / np.square(SourcePs[maxSourceIndex])[0]
        y_scale     = targetPoint[1]*SourcePs[maxSourceIndex][1] / np.square(SourcePs[maxSourceIndex])[1]
        z_scale     = targetPoint[2]*SourcePs[maxSourceIndex][2] / np.square(SourcePs[maxSourceIndex])[2]
